...
export default {
  ...
  mounted() {
    ...
    if (this.allCarts.length === 0||this.isUser.length==0) {
      Swal.fire({
        title: "Checkout",
        text: "Chưa có sản phẩm. Hoặc chưa đăng nhập",
        icon: "error",
    ...
    async placeOrder() {
      try {
        ...
        for (let i = 0; i < this.allCarts.length; i++) {
          ...
          const newPro = await axios.patch(
            `http://localhost:3000/products/${setId}`,
            { quantity: newTonKho }
          );

          const re = await axios.post(
            "http://localhost:3000/chitiethoadons",
            hd
          );
